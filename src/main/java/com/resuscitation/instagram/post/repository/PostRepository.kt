...
interface PostRepository : JpaRepository<Post, Long> {
    ...
    @Query(
        "SELECT p FROM Post p " +
            "WHERE p.idx IN ( " +
            "    SELECT t.postIdx FROM Timeline t " +
            "    WHERE t.nickname IN ( " +
            "        SELECT pr.nickname FROM Profile pr " +
            "        WHERE pr.follower >= 1000) " +
            "    AND t.timestamp > ( " +
            "        SELECT pr.lastAccess FROM Profile pr " +
            "        WHERE pr.nickname = t.nickname ))",
    )
